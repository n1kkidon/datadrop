...
import { Observable, catchError, debounceTime, distinctUntilChanged, mergeMap, of, retry, switchMap } from 'rxjs';
...
export class UtilsService {
  ...
  constructor(private snackbar: MatSnackBar, private httpClient: HttpClient, private permissionsService: NgxPermissionsService, private router: Router,
    private dialog: MatDialog, private guestService: GuestService) {
   }
  ...
  reLogin() : Observable<HttpResponse<LoginResponse> | HttpErrorResponse>{
    ...
    return this.httpClient.post<LoginResponse>('/guest/re-login', {refreshToken}, {observe: 'response', responseType: 'json'});
  }
  ...
  checkIfNameAvailable(control: AbstractControl): Observable<ValidationErrors | null> {
