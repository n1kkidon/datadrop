...
export class RegisterFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    const passwordMatchValidator = (
      ...
      return password === repeatPassword || !this.hide
        ? null
    ...
    this.registerForm = new FormGroup(
      {
        ...
        repeatPass: new FormControl('', [
          this.hide ? Validators.minLength(7) : Validators.nullValidator,
        ]),
      },
  ...
  onSubmit() {
    ...
    if (this.registerForm.valid) {
      ...
      this.guestService
        .register(formData.email, formData.username, formData.pass)
        .subscribe((resp) => {
          if (resp.ok) {
