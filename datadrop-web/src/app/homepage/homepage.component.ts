...
export class HomepageComponent implements OnInit{
  ...
  constructor(private directoryService: DirectoryService, private fileService: FileService, private activeRoute: ActivatedRoute, 
    private router: Router, private dialog: MatDialog, private utilsService: UtilsService, private adminService: AdminService,
  ...
  ngOnInit(): void {
    this.activeRoute.paramMap.subscribe(async param => {
      ...
      else if(this.sharingMode){ //admin cant use this currently
  ...
  getCurrentFolder(id: number){ //TODO: make this recognize what type of fetch it needs (admin, shared, normal)
  ...
  getSharedFolders(userId: number){
    this.userService.getDirectoriesSharedWithCurrentUser(userId).subscribe(response => {
      if(response.ok){
    ...
    this.userService.getFilesSharedWithCurrentUser(userId).subscribe(response => {
      if(response.ok){
  ...
  openFileUploadDialog(){
    ...
    uploadDialog.afterClosed().subscribe((data: FileInfo) => {
      ...
      if(!data || data.parentDirectoryId != this.currentDir.id)
        return;
      this.getCurrentFolder(this.currentDir.id);
    });
  }
  ...
  openEditFormDialog(selectedDir?: DirectoryInfo | FileInfo){
    const fileDialog = this.dialog.open(EditFormDialogComponent, {
      data: selectedDir ? {dir: this.currentDir, editing: selectedDir} : {dir: this.currentDir, editing: undefined},
      disableClose: false,
  ...
  deleteFile(file: FileInfo | DirectoryInfo) {
    this.utilsService.openConfirmActionDialog("Are you sure you want to delete \"" + file.name + "\" ?")
    .afterClosed()
    .subscribe((confirmed: boolean) => {
      if(confirmed){
        if((file as FileInfo).mimeType){
          this.fileService.deleteFileById(file.id).subscribe(resp => this.deletionResponse(file.name, resp.ok));
        }
        else{
          this.directoryService.deleteDirectory(file.id).subscribe(resp => this.deletionResponse(file.name, resp.ok));
        }
      }
    });
  }
  ...
  doubleClickNavigate(id: number): void {
    if(this.adminMode)
      this.utilsService.doubleClick(() => this.router.navigate(['/users/' + this.userId + '/' + id]));
    else if(this.sharingMode)
      this.utilsService.doubleClick(() => this.router.navigate(['/shared/' + this.userId + '/' + id]));
    else this.utilsService.doubleClick(() => this.router.navigate(['/home/' + id]));
  }
}
